...
EpcEnbApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EpcEnbApplication")
    ...
    .AddTraceSource ("RxFromS1u",
                     ...
                     "ns3::EpcEnbApplication::RxTracedCallback")
    .AddTraceSource ("RxFromS1u_split",
                     "Receive data packets from S1-U Net Device",
                     MakeTraceSourceAccessor (&EpcEnbApplication::m_rxS1uSocketPktTrace_split),
                     "ns3::EpcEnbApplication::RxTracedCallback")
    ;
  return tid;
}
...
EpcEnbApplication::RecvFromS1uSocket (Ptr<Socket> socket)
{
 ...
 // uint32_t totalLength = packet->GetSize();
  ...
  if (totalLength > 52)
  {
   // std::cout<<"Receive Total Length:    " << totalLength<<std::endl;
   // f = 1;
  }
  ...
  while (true) {
    ...
    subPacket = packet->CreateFragment(0, gtpuLength-4);
    
    packet->RemoveAtStart(gtpuLength-4);
   ...
   // }
    ...
    //subPacket->PeekHeader(gtpu);
    uint32_t teid = gtpu.GetTeid ();
    std::map<uint32_t, EpsFlowId_t>::iterator it = m_teidRbidMap.find (teid);
    NS_ASSERT (it != m_teidRbidMap.end ());

    m_rxS1uSocketPktTrace_split (subPacket->Copy ());
    SendToLteSocket (subPacket, it->second.m_rnti, it->second.m_bid);
