...
public:
  PC_Queue(){
        ...
        this->a = 1;//length increase/decrease rate
        this->b = 1;//waittime increase/decrease
...
//  typedef void (* RxTracedCallback2)
//    (Ptr<Packet> packet, uint16_t merge_num);
//******************************************
...
static void record_merged_packet(Ptr<Packet> p , uint16_t merge_num);
...
//*********************************************
  ...
  bool ept[256];
//Hash table function
  int find_insert_teid(uint32_t teid,bool &newh){
    uint8_t ini = teid;
    for(int i = 0;i < 256; i++){
...
public:
  ...
  */
  ...
  // static TracedCallback<Ptr<Packet>> m_rxTunPktTrace2;
